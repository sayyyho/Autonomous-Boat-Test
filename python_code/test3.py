...
class Phase1Navigator(Node):
    ...
    def navigation_stage_process(self, red_bbs, green_bbs, yellow_bb, color_img):
        ...
        if red_bbs and green_bbs:
            ...
            if gate_pair:
                ...
                cv2.putText(color_img, f"GATE #{self.gates_passed+1} DETECTED", (20, 40), 
            ...
            cx = bb[0] + bb[2]//2
            cv2.putText(color_img, f"SCANNING for {visible_color} pair", (20, 40), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        ...
        # ⭐ 노란 부표 감지 시 미션 단계 전환
        elif yellow_bb and self.gates_passed >= 3:  # 최소 3개 게이트 통과 후
            ...
            # 부표가 아예 없을 시 
            cv2.putText(color_img, "Searching for gates...", (20, 40), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    def manage_gate_passing(self, gate_center: Tuple[int, int], frame: np.ndarray):
        ...
        if gate_y > COLOR_H * 0.7:  # 화면 하단 70% 이상
            if self.gate_passing_state != 'PASSING':
        ...
        elif self.gate_passing_state == 'PASSING' and gate_y < COLOR_H * 0.3:
            # 새로운 게이트가 화면 상단에 나타남 = 이전 게이트 통과 완료
            self.gates_passed += 1
            self.gate_passing_state = 'APPROACHING'
            self.get_logger().info(f"✅ 게이트 #{self.gates_passed} 통과 완료!")
            ...
            if abs(error) <= GATE_CENTER_DEADZONE:
                self.get_logger().info("게이트 중앙 정렬 → 전진")
                set_motor_state_named('forward')
                time.sleep(APPROACH_FORWARD_TIME)
            elif error > 0:
