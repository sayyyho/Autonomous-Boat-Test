...
"""
KABOAT Phase1: Practical S-Course Navigation
- 짧은 회전 + 상태 체크
- 카메라 지연 고려
- 실전에서 동작하는 로직
"""
...
GREEN_ONLY_COUNT = 0
RED_ONLY_COUNT = 0
COUNT_THRESHOLD = 2  # 2번 연속 보이면 확정
...
class PracticalNavigator(Node):
    def __init__(self):
        super().__init__('practical_navigator')
        ...
        # ⭐ 상태 카운터
        self.green_only_count = 0
        self.red_only_count = 0
        
        self.get_logger().info("=" * 60)
        self.get_logger().info("🚢 Practical Navigator 시작")
        self.get_logger().info("=" * 60)
    ...
    def process(self):
        ...
        cv2.putText(frame, f"R:{len(reds)} G:{len(greens)} | GC:{self.green_only_count} RC:{self.red_only_count}",
        ...
        if has_red and has_green:
            ...
            if gate:
                cv2.putText(frame, "GATE MODE!", (20, 130),
                           cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 255), 3)
                self.navigate_through_gate(gate, frame, frame_cx)
            else:
                # 유효한 게이트 아님 → 직진
                cv2.putText(frame, "Both visible but no valid gate", (20, 130),
        ...
        # ⭐ 케이스 2: 초록만 보임
            ...
            self.green_only_count += 1
            self.red_only_count = 0
            
            cv2.putText(frame, f"GREEN ONLY (count: {self.green_only_count})", (20, 130),
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)
            
            # ⭐ 2번 이상 연속으로 초록만 보이면 우회전
            if self.green_only_count >= COUNT_THRESHOLD:
                cv2.putText(frame, "→ TURN RIGHT", (20, 170),
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
                self.get_logger().info(f"🟢 초록만 {self.green_only_count}회 → 우회전")
                ...
                # 아직 확정 아님 → 잠시 대기
                cv2.putText(frame, "Waiting for confirmation...", (20, 170),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                time.sleep(0.1)
        
        # ⭐ 케이스 3: 빨강만 보임
        elif has_red and not has_green:
            self.red_only_count += 1
            self.green_only_count = 0
            
            cv2.putText(frame, f"RED ONLY (count: {self.red_only_count})", (20, 130),
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
            
            # ⭐ 2번 이상 연속으로 빨강만 보이면 좌회전
            if self.red_only_count >= COUNT_THRESHOLD:
                cv2.putText(frame, "→ TURN LEFT", (20, 170),
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
                self.get_logger().info(f"🔴 빨강만 {self.red_only_count}회 → 좌회전")
    ...
    def navigate_through_gate(self, gate: Tuple[Dict, Dict], frame: np.ndarray, frame_cx: int):
        ...
        # 접근 중
            ...
            cv2.putText(frame, f"Error: {error}px", (20, 160),
            ...
            if abs(error) <= DEADZONE:
                ...
                time.sleep(0.12 * min(abs(error)/100, 1.5))
...
def main(args=None):
    print("\n" + "=" * 60)
    print("🚢 KABOAT Practical Navigator")
    print("=" * 60)
    print("📋 개선 사항:")
    print("  - 짧은 회전 (0.3초)")
    print("  - 2번 연속 확인 후 행동")
