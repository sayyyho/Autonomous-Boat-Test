...
"""
KABOAT Phase1: Simple S-Course Navigation
- 초록만 보임 → 우회전
- 빨강만 보임 → 좌회전
- 둘 다 보임 → 중앙 직진
"""
...
class SimpleSNavigator(Node):
    def __init__(self):
        super().__init__('simple_s_navigator')
        ...
        self.get_logger().info("🚢 Simple S-Course Navigator 시작")
        self.get_logger().info("=" * 60)
    ...
    def process(self):
        ...
        if has_red and has_green:
            ...
            if gate:
                ...
                self.get_logger().info("→ 콘은 보이나 유효한 게이트 없음 → 직진")
                self.motor.forward()
        ...
        # 케이스 2: 초록만 보임 → 우회전
            ...
            cv2.putText(frame, "GREEN ONLY → TURN RIGHT", (20, 130),
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)
            self.get_logger().info("🟢 초록만 보임 → 우회전!")
        ...
        # 케이스 3: 빨강만 보임 → 좌회전
            ...
            cv2.putText(frame, "RED ONLY → TURN LEFT", (20, 130),
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
            self.get_logger().info("🔴 빨강만 보임 → 좌회전!")
    ...
    def navigate_through_gate(self, gate: Tuple[Dict, Dict], frame: np.ndarray, frame_cx: int):
        ...
        # 접근 중
            ...
            cv2.putText(frame, f"Approaching Gate #{self.gates+1} | Error: {error}px",
                       (20, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
...
def main(args=None):
    print("\n" + "=" * 60)
    print("🚢 KABOAT Simple S-Course Navigator")
    print("=" * 60)
    print("📋 로직:")
    print("  1. 초록만 보임 → 우회전")
    print("  2. 빨강만 보임 → 좌회전")
    print("  3. 둘 다 보임 → 중앙 직진")
    print("=" * 60)
