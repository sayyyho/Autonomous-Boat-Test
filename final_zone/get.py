...
class YoloPerceptionNode(Node):
    ...
    def process_detections(self, result, mode: str):
        ...
        if mode == 'DOCK':
            ...
            self.get_logger().info(
                f"[DOCK] 선택된 도킹 타겟(red_triangle): global=({xg:.2f}, {yg:.2f}, {zg:.2f}), "
                f"conf={best_conf:.2f}"
            )
    ...
    def pixel_to_camera(self, u_color: float, v_color: float, depth: float):
        ...
        """
        # 색상 좌표를 depth 좌표로 변환한 u_depth/v_depth를 이용하려면
        # 여기서는 단순히 cx, cy, fx, fy 를 depth 카메라 기준으로 사용.
        # u_color, v_color는 이미 depth 해상도로 스케일링된 걸 사용해도 되고,
        # 지금은 get_depth_for_color_pixel에서 depth 픽셀을 썼으니
        # 근사적으로 color 기준으로 계산해도 큰 차이 없음.
