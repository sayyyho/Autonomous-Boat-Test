...
"""
Phase1: Green Priority Detection
- 꼬깔에 초록이 있으면 무조건 초록 부표로 인식
- 초록과 겹치는 빨강 영역은 무시
- 수평 정렬된 게이트만 유효
"""
...
# ⭐ 초록 우선 판단을 위한 겹침 확장 계수
OVERLAP_EXPANSION = 1.5  # 초록 영역 1.5배 확장하여 겹침 판단
...
class Phase1Navigator(Node):
    def __init__(self):
        ...
        self.get_logger().info("=== Phase1 Navigator (Green Priority) 시작 ===")
    ...
    def station_keeping_stage(self, yellow_bb, frame):
        ...
        if yellow_bb:
            ...
            if 0.1 < depth < YELLOW_STOP_DISTANCE:
                self.get_logger().info(f"🟡 노란부표 {depth:.2f}m 도달 → {YELLOW_WAIT_TIME}초 대기")
                motor_action('stop')
