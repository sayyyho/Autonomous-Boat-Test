...
"""
Phase2: Search Mission - 목표 색상 부표 중심 선회
- 목표 색상 부표 탐지 및 접근
- 일정 거리 도달 시 선회 시작
- 빨강/초록: 시계방향 / 파랑: 반시계방향
- IMU 기반 360도 회전 완료 판단
"""
...
CIRCLE_RADIUS_TARGET = 2.5  # 목표 선회 반경 (미터)
CIRCLE_SPEED = '4'  # 선회 속도

# 회전 각도 임계값 (360도 ± 여유)
ROTATION_THRESHOLD = 350  # 도
ROTATION_MARGIN = 20  # 도 (오버슈트 허용)
...
class RotationTracker:
    ...
    def reset(self, initial_yaw: float):
        """선회 시작 시 초기화"""
        ...
        print(f"🧭 회전 추적 시작: 초기 yaw = {np.degrees(initial_yaw):.1f}°")
    
    def update(self, yaw: float):
        """현재 yaw 값으로 총 회전량 업데이트"""
        if self.last_yaw is None:
            self.last_yaw = yaw
            return
        
        # yaw 차이 계산 (-π ~ π 범위로 정규화)
        ...
        self.total_rotation += abs(delta)  # 절대값으로 누적
    ...
    def get_rotation_degrees(self) -> float:
        """총 회전 각도 (도 단위)"""
        return np.degrees(self.total_rotation)
    
    def is_full_circle(self) -> bool:
        """360도 회전 완료 여부"""
        rotation_deg = self.get_rotation_degrees()
        return rotation_deg >= ROTATION_THRESHOLD
...
class SearchNavigator(Node):
    ...
    def imu_callback(self, msg: Imu):
        ...
        if self.mission_stage == 'CIRCLING':
            ...
            self.rotation_tracker.update(yaw)
    ...
    def process_frame(self):
        ...
        if self.mission_stage == 'CIRCLING':
            rotation = self.rotation_tracker.get_rotation_degrees()
            cv2.putText(frame, f"Rotation: {rotation:.1f} / {ROTATION_THRESHOLD}", 
    ...
    def start_circling(self):
        ...
        if self.current_orientation:
            ...
            euler = rotation.as_euler('xyz', degrees=False)
            initial_yaw = euler[2]
            self.rotation_tracker.reset(initial_yaw)
    ...
    def circling_stage(self, target_bb, frame):
        ...
        # ⭐ 360도 회전 완료 체크
        if self.rotation_tracker.is_full_circle():
            rotation = self.rotation_tracker.get_rotation_degrees()
            elapsed = time.time() - self.circle_start_time
            self.get_logger().info(f"✅ 선회 완료! 총 회전: {rotation:.1f}° / 시간: {elapsed:.1f}초")
        ...
        cv2.putText(frame, f"Circling: {rotation:.1f}° / Time: {elapsed:.1f}s", 
        ...
        if target_bb:
            x, y, w, h = target_bb
            cx = x + w // 2
            depth = self.get_depth(cx, y + h // 2)
            
            if 0.1 < depth < CIRCLE_RADIUS_TARGET - 0.5:
                # 너무 가까우면 잠시 후진
